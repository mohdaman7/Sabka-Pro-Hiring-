...
export function useLeadViewModel() {
  ...
  const createLead = useCallback(async (leadData) => {
    ...
    try {
      ...
      if (!newLead.isValid()) {
        throw new Error(
          "Invalid lead data. First name, last name, and email are required."
        );
      }
  ...
  // Auto-assign leads via round-robin
  const autoAssignLeads = useCallback(
    async (leadIds = []) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        if (data.success) {
          ...
          throw new Error(data.message || "Failed to auto-assign leads");
        }
      } catch (err) {
        const errorMessage =
  ...
  const fetchLeadById = useCallback(async (leadId) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || err.message || "Failed to fetch lead";
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const unassignLead = useCallback(async (leadId) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || err.message || "Failed to unassign lead";
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const addFollowUp = useCallback(async (leadId, followUpData) => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || err.message || "Failed to add follow-up";
      setError(errorMessage);
      return { success: false, error: errorMessage };
    } finally {
      setLoading(false);
  ...
  const convertLead = useCallback(
    async (leadId, convertedTo = "student", conversionValue = 0) => {
      setLoading(true);
      setError(null);
