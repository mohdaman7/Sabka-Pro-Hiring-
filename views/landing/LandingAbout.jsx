...
export default function AboutPage() {
  ...
  return (
    <div className="min-h-screen relative bg-transparent">
      ...
      {/* Pillars Section */}
      <section className="relative py-12 sm:py-16 lg:py-20 px-4 sm:px-6 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 lg:gap-12 select-text">
          {pillars.map(
            (
              {
                icon: Icon,
              ...
              >
                ...
                <div
                  className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl sm:rounded-3xl flex items-center justify-center mb-4 sm:mb-6 bg-gradient-to-br ${gradientFrom} ${gradientTo} shadow-lg text-white transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6`}
                >
                  <Icon className="w-7 h-7 sm:w-8 sm:h-8" />
                </div>
                <h3
                  id={`pillar-title-${i}`}
                  className="text-xl sm:text-2xl font-extrabold text-white mb-3 sm:mb-4 tracking-tight"
                >
                  {title}
                </h3>
                <p
                  id={`pillar-desc-${i}`}
                  className="text-sm sm:text-base text-purple-200 leading-relaxed font-light"
                >
                  {description}
                </p>
              </article>
      ...
      {/* Our Story Section */}
      <section className="relative py-12 sm:py-16 lg:py-20 px-4 sm:px-6 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 select-text">
        {/* Text Content */}
        <div className="flex flex-col justify-center">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4 sm:mb-6 tracking-tight leading-snug">
            Our Story
          </h2>
          <div className="space-y-4 sm:space-y-5 text-purple-300 font-light leading-relaxed text-sm sm:text-base lg:text-lg">
            <p>
          ...
          <div className="mt-6 sm:mt-8 flex flex-wrap gap-3 sm:gap-4">
            {[
              ...
              <div
                key={i}
                className="flex items-center gap-2 rounded-lg px-3 sm:px-5 py-1.5 sm:py-2 bg-white/10 backdrop-blur-md border border-white/20"
              >
                <CheckCircle2 className={`w-4 h-4 sm:w-5 sm:h-5 ${iconColor}`} />
                <span className="text-purple-200 font-semibold text-xs sm:text-sm">{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div
          className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-white/5 backdrop-blur-xl"
          aria-label="Team collaboration and milestones"
        >
          <img
            src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&auto=format&fit=crop"
            alt="Team collaboration"
            className="w-full h-64 sm:h-80 lg:h-96 object-cover rounded-t-2xl sm:rounded-t-3xl"
            loading="lazy"
            decoding="async"
          />
          <div className="grid grid-cols-3 text-center p-4 sm:p-6 bg-gradient-to-t from-black/70 to-transparent gap-4 sm:gap-6">
            <div>
              <p className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">
                2020
              </p>
              <p className="text-xs sm:text-sm text-purple-300 font-medium tracking-wide">
                Founded
              </p>
            </div>
            <div>
            ...
            <div>
              <p className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-pink-500">
                500+
              </p>
              <p className="text-xs sm:text-sm text-purple-300 font-medium tracking-wide">
                Partners
