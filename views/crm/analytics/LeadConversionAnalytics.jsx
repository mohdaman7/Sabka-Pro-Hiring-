      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-6 shadow-lg border-2 border-gray-100">
        ...
        <div className="space-y-4">
          {data.conversionBySource.map((source) => (
            <div key={source._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
              <div>
                <p className="font-semibold text-gray-900 capitalize">{source.source || "Unknown"}</p>
                <p className="text-sm text-gray-600">{formatNumber(source.total)} leads</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-green-600">{formatPercentage(source.conversionRate)}</p>
                <p className="text-xs text-gray-500">{formatNumber(source.converted)} converted</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-6 shadow-lg border-2 border-gray-100">
        ...
        <div className="space-y-3">
          {data.staffPerformance.slice(0, 5).map((staff, index) => (
            <div key={staff.staffId} className="flex items-center gap-4 p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl">
              <div className="w-10 h-10 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold">
                #{index + 1}
              </div>
              <div className="flex-1">
                <p className="font-semibold text-gray-900">Staff {staff.staffId}</p>
                <p className="text-sm text-gray-600">{formatNumber(staff.totalLeads)} leads</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-purple-600">{formatPercentage(staff.conversionRate)}</p>
                <p className="text-xs text-gray-500">{formatNumber(staff.converted)} converted</p>
              </div>
            </div>
          ))}
        </div>
