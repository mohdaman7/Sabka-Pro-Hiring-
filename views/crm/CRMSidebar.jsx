    ...
    children: [
      {
        id: "ats-applications",
        name: "Applications",
        href: "/crm/ats-management/applications",
        icon: FileText,
  ...
  return (
    <>
      ...
      >
        ...
        {/* Header */}
        <div className="relative h-24 flex items-center justify-between px-5 border-b border-white/10 shrink-0 bg-gradient-to-r from-white/5 to-transparent">
          <div className="flex items-center gap-3">
            {!isCollapsed ? (
              <>
                <div className="relative group cursor-pointer">
                  <div
                    className="absolute inset-0 rounded-2xl blur-xl opacity-60 group-hover:opacity-90 transition-all duration-500 animate-pulse"
                    style={{
                      background: "linear-gradient(135deg, #803791, #b87bd1)",
                    }}
                  />
                  <div
                    className="relative w-12 h-12 rounded-2xl flex items-center justify-center shadow-2xl border-2 border-white/20 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500"
                    style={{
                      background: "linear-gradient(135deg, #803791, #b87bd1)",
                    }}
                  >
                    <BarChart3
                      className="w-6 h-6 text-white group-hover:scale-110 transition-transform duration-500"
                      strokeWidth={2.5}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h1 className="text-lg font-bold text-white tracking-tight bg-gradient-to-r from-white to-white/90 bg-clip-text">
                      Sabka Pro CRM
                    </h1>
              ...
              <div className="relative group cursor-pointer">
                ...
                >
                  <BarChart3 className="w-6 h-6 text-white group-hover:scale-110 transition-transform duration-500" strokeWidth={2.5} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden md:flex relative group w-10 h-10 rounded-xl bg-gradient-to-br from-white/10 to-white/5 hover:from-white/15 hover:to-white/10 items-center justify-center transition-all duration-300 border-2 border-white/10 hover:border-white/20 shadow-lg hover:shadow-xl hover:scale-110"
            >
              <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-purple-500/20 to-pink-500/20 opacity-0 group-hover:opacity-100 transition-opacity blur-sm" />
              {isCollapsed ? (
                <ChevronRight className="w-5 h-5 text-white relative z-10 group-hover:translate-x-0.5 transition-transform" />
              ) : (
                <ChevronLeft className="w-5 h-5 text-white relative z-10 group-hover:-translate-x-0.5 transition-transform" />
              )}
            </button>
            {isMobile && (
              <button
                onClick={onClose}
                className="md:hidden w-10 h-10 rounded-xl bg-gradient-to-br from-rose-500/20 to-pink-500/20 hover:from-rose-500/30 hover:to-pink-500/30 flex items-center justify-center transition-all duration-300 border-2 border-rose-400/20 hover:border-rose-400/40 shadow-lg hover:scale-110"
              >
            ...
            return (
              <div key={item.id} className="space-y-1">
                ...
                {hasChildren && isExpanded && !isCollapsed && (
                  <div className="ml-5 pl-4 border-l-2 border-white/10 space-y-1.5 py-2 relative">
                    {/* Gradient line accent */}
                    <div 
                      className="absolute left-0 top-0 bottom-0 w-0.5 rounded-r-full opacity-50"
                      style={{
                    ...
                    {item.children.map((child) => {
                      ...
                      return (
                        <Link
                          ...
                          className={`relative w-full group flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-300 border-2 ${
                            childActive
                              ? "bg-gradient-to-r from-white/10 to-white/5 border-white/20 shadow-lg"
                              : "bg-white/0 border-white/5 hover:bg-white/5 hover:border-white/15"
                          }`}
                        >
                          {/* Active indicator */}
                          {childActive && (
                            <div
                              ...
                              style={{
                                background: `linear-gradient(to bottom, ${colors.from}, ${colors.to})`,
                                boxShadow: `0 0 12px ${colors.from}60`,
                              }}
                            />
                          )}
                          ...
                          <div
                            className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 rounded-r-full transition-all duration-300 ${
                              childActive ? "h-0" : "h-0 group-hover:h-6"
                            }`}
                            style={{
                              background: `linear-gradient(to bottom, ${colors.from}, ${colors.to})`,
                            }}
                          ...
                          <div className="relative">
                            ...
                            <div
                              className={`relative w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 shrink-0 border-2 ${
                                childActive 
                                  ? "scale-110 shadow-lg" 
                            ...
                            >
                              <ChildIcon
                                className={`w-4 h-4 transition-all ${
                                  childActive ? "text-white" : "text-white/60 group-hover:text-white/80"
                                }`}
                          ...
                          {childActive && (
                            <div
                              ...
                              style={{
                                background: `linear-gradient(135deg, ${colors.from}, ${colors.to})`,
                                boxShadow: `0 0 12px ${colors.from}80`,
                              }}
                            />
                          )}
                        </Link>
